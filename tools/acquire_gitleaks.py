...
def _copy_response(response: HttpResponse, handle: Any, length: int) -> int:
    received = 0
    while True:
...
def _stream_to_file(response: HttpResponse, destination: Path, length: int) -> None:
    ...
    try:
        with handle:
            received = _copy_response(response, handle, length)
        if received != length:
...
def extract_gitleaks(archive: Path, directory: Path) -> Path:
    ...
    try:
        ...
        primary = AcquisitionError(f"cannot write the extracted executable: {error.strerror}")
        _remove(target, primary)
        raise primary from error
