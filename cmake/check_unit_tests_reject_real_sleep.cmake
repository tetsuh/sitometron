...
function(_sleep_scan_content _path _content _out_found _out_token)
  ...
  if(_family STREQUAL cpp)
    if(_normalized MATCHES "(^|[^A-Za-z0-9_])std::this_thread::sleep_for([^A-Za-z0-9_]|$)")
...
if(DEFINED SITOMETRON_POLICY_SELF_TEST_MODE AND SITOMETRON_POLICY_SELF_TEST_MODE)
  ...
  endforeach()
  foreach(_separator IN ITEMS "\t" "\n" "\r\n")
    foreach(_token IN ITEMS "::Sleep" "usleep" "nanosleep" "sleep")
      _sleep_expect_child_finding(
        "C++ ${_token} separator" "tests/unit/sample.cpp"
        "${_token}${_separator}(1)" "${_cpp_clean}")
    endforeach()
    _sleep_expect_child_finding(
      "Python time.sleep separator" "tests/tooling/sample.py"
      "time.sleep${_separator}(1)" "${_python_clean}")
  endforeach()
  _sleep_expect_child_finding(
