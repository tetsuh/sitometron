...
| `NFR-004` | CTest `sitometron_core_tests`; planned CTests `core_port_fake_contracts`, `job_fake_effect_observation`; policy check `unit_tests_reject_real_sleep` | Core test active; port/fake support checks planned under Accepted ADR-0005 and Issue #11; policy check planned before the Phase 0A Gate closes |
