...
| ID | Level | Requirement | Authority |
|---|---|---|---|
| `JRN-004` | MUST | Encode each Journal event as one canonical UTF-8 NDJSON record through one serializer and its exact inverse parser, and reject before writing any event whose encoding exceeds the record size bound. | Proposed [ADR-0006](adr/0006-define-physical-jobjournal-durability-contract.md) under [Issue #51](https://github.com/tetsuh/sitometron/issues/51) |
| `JRN-005` | MUST | Report `kCommitted` only after the complete record and its LF are written and data-synced, and make each new segment's file and directory entry durable before its first record commits, returning definite failure when that cannot be done. | Proposed [ADR-0006](adr/0006-define-physical-jobjournal-durability-contract.md) under [Issue #51](https://github.com/tetsuh/sitometron/issues/51) |
...
| `OPS-002` | MUST | Validate every segment before the writer starts and refuse to start on a torn tail, corruption, replay capacity overflow, or sequence exhaustion, accept an empty highest segment named for the next sequence, and never modify the Journal. | Proposed [ADR-0006](adr/0006-define-physical-jobjournal-durability-contract.md) under [Issue #51](https://github.com/tetsuh/sitometron/issues/51) |
