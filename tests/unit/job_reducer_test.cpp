...
int Run() {
  ...
  int result = 0;
  result |= Check(absent.state == JobState::kAbsent, "initial entity position is absent");
  const RawCandidateEvent created{1, job, "job_created", "{\"session_id\":\"" + std::string(kJob) + "\"}"};
  const Decision created_decision = NormalizeCandidate(absent, created);
  ...
  const auto allocation_decision = NormalizeCandidate(absent, allocation);
...
int main(int argc, char** argv) {
  int result = Run();
  if (argc > 1) result |= sitometron::test::RunJobReducerVectorChecks(argv[1]);
  return result;
}
