...
try {
  ...
  function Invoke-Wrapper([string]$Repo,[hashtable]$ExtraEnv=@{},[string]$PathOverride=$null,[bool]$InProcess=$false,[bool]$ShadowCommands=$false) {
    $shadowOriginal=@{}
    $save = @{}; foreach ($n in @('Path','FAKE_LOG','FAKE_FAIL_STAGE','FAKE_GIT_ORIGIN','FAKE_GIT_REV','FAKE_GIT_STATUS','VSCMD_ARG_TGT_ARCH','FAKE_BUILD_TREE','FAKE_CMAKE_COMPILER','FAKE_TOOLCHAIN','FAKE_INSTALLED','VCPKG_DEFAULT_TRIPLET','VCPKG_ROOT','VCPKG_DOWNLOADS','VCPKG_DISABLE_METRICS','GIT_TERMINAL_PROMPT','GCM_INTERACTIVE')) { $save[$n] = [Environment]::GetEnvironmentVariable($n) }
    try {
