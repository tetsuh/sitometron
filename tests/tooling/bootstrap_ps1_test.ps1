...
try {
  ...
  function bootstrap_missing_host_tool {foreach($tool in 'cmake','ctest'){$r=Copy-Case "missing-host-tool-$tool";Write-Pin $r;Write-Fake-Tools $r;Set-Content -LiteralPath (Join-Path $r "fake-native-bin/$tool.cmd") -Value @('@echo off',"echo $tool fixture unavailable 1>&2",'exit /b 127');$s=Invoke-Wrapper $r;if($s-eq0-or$script:WrapperOutput-notmatch"$tool|missing|tool|version|failed"){Fail "missing host tool/$tool failed to reject: $script:WrapperOutput"}};Pass $MyInvocation.MyCommand.Name}
  ...
  function bootstrap_forbidden_environment {$r=Copy-Case forbidden-environment;Write-Pin $r;Write-Fake-Tools $r;$s=Invoke-Wrapper $r @{VCPKG_DEFAULT_TRIPLET='bad'};if($s-eq0 -or $script:WrapperOutput-notmatch'environment|selection'){Fail 'forbidden environment was accepted'};$l=Get-Content (Join-Path $r 'fake-native.log') -Raw;if($l-match'git .*clone|bootstrap-vcpkg|vcpkg install|cmake --preset|cmake --build|ctest --preset'){Fail 'forbidden environment performed checkout or build work'};Pass $MyInvocation.MyCommand.Name}
  function bootstrap_cache_mismatch {$r=Copy-Case cache-mismatch;Write-Pin $r;Write-Fake-Tools $r;if((Invoke-Wrapper $r)-ne0){Fail 'cache setup failed'};Set-Content (Join-Path $r 'fake-native.log') '';Set-Content (Join-Path $r 'build/dev-windows/CMakeCache.txt') 'CMAKE_C_COMPILER:FILEPATH=C:/wrong/compiler';$s=Invoke-Wrapper $r;if($s-eq0 -or $script:WrapperOutput-notmatch'cache|identity|mismatch'){Fail 'cache mismatch was accepted'};if((Get-Content (Join-Path $r 'fake-native.log') -Raw)-match'cmake --preset'){Fail 'cache mismatch reached configure'};$r=Copy-Case cache-redirection;Write-Pin $r;Write-Fake-Tools $r;$cache=Join-Path $r 'build/dev-windows/CMakeCache.txt';$outside=Join-Path $r '..\outside-cache';New-Item -ItemType Directory (Split-Path $cache) -Force|Out-Null;New-Item -ItemType Directory $outside -Force|Out-Null;Set-Content (Join-Path $outside sentinel) keep;New-Item -ItemType Junction -Path $cache -Target $outside|Out-Null;$s=Invoke-Wrapper $r;if($s-eq0-or$script:WrapperOutput-notmatch'cache|managed|reparse|regular file'){Fail 'cache redirection was accepted'};if(-not(Test-Path (Join-Path $outside sentinel))){Fail 'cache redirection target was mutated'};if((Get-Content (Join-Path $r 'fake-native.log') -Raw)-match'cmake --preset'){Fail 'cache redirection reached configure'};Pass $MyInvocation.MyCommand.Name}
